...
// Popup de telefono: expone la URL AJAX y procesa solicitudes cortas.
add_action('wp_footer', function () {
    ...
    $config = [
        ...
        'nonce' => wp_create_nonce('skynet_popup_contact'),
    ];

    echo '<script>window.skynetPopupContact=' . wp_json_encode($config) . ';</script>' . "\n";
}, 20);

add_action('wp_ajax_skynet_popup_contact', 'skynet_handle_popup_contact_request');
add_action('wp_ajax_nopriv_skynet_popup_contact', 'skynet_handle_popup_contact_request');

if (!function_exists('skynet_handle_popup_contact_request')) {
    function skynet_handle_popup_contact_request()
    {
        $nonce = isset($_POST['nonce']) ? sanitize_text_field(wp_unslash($_POST['nonce'])) : '';

        if (!wp_verify_nonce($nonce, 'skynet_popup_contact')) {
            wp_send_json_error(
                ['message' => 'No se ha podido validar la solicitud. Recarga la pagina e intentalo de nuevo.'],
                403
            );
        }
        ...
        $phone = isset($_POST['telefono']) ? trim(sanitize_text_field(wp_unslash($_POST['telefono']))) : '';
        $page = isset($_POST['pagina']) ? esc_url_raw(wp_unslash($_POST['pagina'])) : home_url('/');

        if (!preg_match('/^[0-9+\s().-]{6,25}$/', $phone)) {
            wp_send_json_error(
                ['message' => 'Introduce un numero de telefono valido.'],
                400
            );
        }

        $ip = isset($_SERVER['REMOTE_ADDR']) ? sanitize_text_field(wp_unslash($_SERVER['REMOTE_ADDR'])) : '';
        $rate_key = 'skynet_popup_contact_' . md5($ip);

        if (get_transient($rate_key)) {
            wp_send_json_error(
                ['message' => 'Ya hemos recibido tu solicitud. Espera un minuto antes de intentarlo de nuevo.'],
                429
            );
        }
        ...
        if (!$sent) {
            wp_send_json_error(
                ['message' => 'No se ha podido enviar. Intentalo de nuevo.'],
                500
            );
        }
